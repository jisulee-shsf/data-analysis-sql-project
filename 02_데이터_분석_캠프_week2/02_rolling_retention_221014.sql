...
-- 3-2. 첫 구매한 달 이후 이탈하지 않은 고객 비율 출력하기
SELECT first_order_month
    , COUNT(DISTINCT customer_id) * 100 / COUNT(DISTINCT customer_id) AS month0
    , FORMAT(COUNT(DISTINCT CASE WHEN DATE_ADD(first_order_month, INTERVAL 1 month) <= last_order_month THEN customer_id END) * 100 / COUNT(DISTINCT customer_id), 2) AS month1
    , FORMAT(COUNT(DISTINCT CASE WHEN DATE_ADD(first_order_month, INTERVAL 2 month) <= last_order_month THEN customer_id END) * 100 / COUNT(DISTINCT customer_id), 2)  AS month2
    , FORMAT(COUNT(DISTINCT CASE WHEN DATE_ADD(first_order_month, INTERVAL 3 month) <= last_order_month THEN customer_id END) * 100 / COUNT(DISTINCT customer_id), 2)  AS month3
    , FORMAT(COUNT(DISTINCT CASE WHEN DATE_ADD(first_order_month, INTERVAL 4 month) <= last_order_month THEN customer_id END) * 100 / COUNT(DISTINCT customer_id), 2)  AS month4
    , FORMAT(COUNT(DISTINCT CASE WHEN DATE_ADD(first_order_month, INTERVAL 5 month) <= last_order_month THEN customer_id END) * 100 / COUNT(DISTINCT customer_id), 2)  AS month5
    , FORMAT(COUNT(DISTINCT CASE WHEN DATE_ADD(first_order_month, INTERVAL 6 month) <= last_order_month THEN customer_id END) * 100 / COUNT(DISTINCT customer_id), 2)  AS month6
    , FORMAT(COUNT(DISTINCT CASE WHEN DATE_ADD(first_order_month, INTERVAL 7 month) <= last_order_month THEN customer_id END) * 100 / COUNT(DISTINCT customer_id), 2) AS month7
    , FORMAT(COUNT(DISTINCT CASE WHEN DATE_ADD(first_order_month, INTERVAL 8 month) <= last_order_month THEN customer_id END) * 100 / COUNT(DISTINCT customer_id), 2)  AS month8
    , FORMAT(COUNT(DISTINCT CASE WHEN DATE_ADD(first_order_month, INTERVAL 9 month) <= last_order_month THEN customer_id END) * 100 / COUNT(DISTINCT customer_id), 2)  AS month9
    , FORMAT(COUNT(DISTINCT CASE WHEN DATE_ADD(first_order_month, INTERVAL 10 month) <= last_order_month THEN customer_id END) * 100 / COUNT(DISTINCT customer_id), 2)  AS month10
    , FORMAT(COUNT(DISTINCT CASE WHEN DATE_ADD(first_order_month, INTERVAL 11 month) <= last_order_month THEN customer_id END) * 100 / COUNT(DISTINCT customer_id), 2)  AS month11
FROM records_preprocessed
GROUP BY 1;
